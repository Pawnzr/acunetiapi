...
def acu():
    data = {}
    url_acunetix = "https://127.0.0.1:3443/api/v1/"
    x_auth = "1986ad8c0a5b3df4d7028d5f3c06e936c1ccc3912274a488596a4478221ae1d8c"
...
def get_result(folder_name):
    ...
    try:
        ...
        progress = data_status.get("scanning_app", {}).get("wvs", {}).get("main", {}).get("progress")
        severity_counts = data_status.get("severity_counts")
        
        exclude_fields = ["app", "loc_id", "status", "target_vuln_id","vt_created", "vt_id", "vt_updated", "vuln_id"]


        filtered_vulnerabilities = []
        for vulnerability in vulnerabilities:
            filtered_vulnerability = {key: value for key, value in vulnerability.items() if key not in exclude_fields}
...
def get_session_id(url_acunetix,scan_id,x_auth):
    headers = {"X-Auth": x_auth,"Cookie": "ui_session={}".format(x_auth),"Referer": "https://127.0.0.1:3443/api/v1/"}
